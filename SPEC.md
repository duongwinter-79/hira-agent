...
| **M1.5.c-2 — Spec/ADR delta state machine** | Architect ADRs become `delta` artifacts in `.hira/runs/<run_id>/deltas/` rather than auto-folding into memory; `hira runs approve <run_id>` folds deltas into the baseline (memory + the worktree branch merge). Failed/unapproved Runs leave the baseline untouched (§4.8). |

...
| **M1.5.b — Real Developer edits + hard gate** | Developer writes to an isolated git worktree (`.hira/runs/<run_id>/worktree/`); the engine verifies the *actual* diff; a `fail` report hard-gates the Developer→Reviewer hand-off and routes back to the Developer with the tool output attached, with a retry counter in the journal. |
| **M1.5.c — Spec lifecycle & traceability** | Spec/ADR delta state machine (§4.8); `spec-consistency` MCP skill for the Cross-Artifact Consistency pass; bidirectional traceability view (`hira runs trace`, §4.9). |

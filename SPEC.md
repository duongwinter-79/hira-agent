...
| **M1.3.b — Developer / Tester / Reviewer prompts** | Wire the remaining three specialists with read-only mode for now (Edit / Write / Bash blocked via the executor's tool override). Each emits a structured artifact (proposed patch, test plan, review verdict). Real file mutation lifts in M1.5 once the deterministic Verification Engine is in place. |
